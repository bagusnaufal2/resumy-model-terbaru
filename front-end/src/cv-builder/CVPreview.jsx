          ...
          {contactItems.length > 0
            ? contactItems.map((item, index) => (
                <span
                  ...
                  className='cv-preview-contact-item'
                >
                  {index > 0 && ' | '}
                  {item.type === 'link' ? (
                    <a
            ...
            <div key={index} className='cv-preview-entry'>
              <div className='cv-preview-entry-head'>
                <div>
                  ...
                  <p className='cv-preview-company'>
                    {exp.company || 'Company Name'}
                  </p>
                </div>
                <span>
                  {formatDate(exp.startDate) || 'Start'} -{' '}
                  {exp.isCurrent ? 'Present' : formatDate(exp.endDate) || 'End'}
                </span>
              </div>
          ...
          <div key={index} className='cv-preview-entry'>
            <div className='cv-preview-entry-head'>
              ...
              <span>
                ...
                {edu.isCurrent ? 'Present' : formatDate(edu.endDate) || 'End'}
              </span>
            </div>
                ...
                <div className='cv-preview-entry-head'>
                  <div>
                    ...
                    {certificate.certificateLink && (
                      <a
                        href={normalizeUrl(certificate.certificateLink)}
                        target='_blank'
                        rel='noreferrer'
                        className='cv-preview-link'
                      >
                        Certificate: {certificate.certificateLink}
                      </a>
                    )}
                  </div>
                  {(certificate.startDate || certificate.endDate) && (
                    <span>
                      {formatDate(certificate.startDate) || 'Start'} -{' '}
                      {certificate.isCurrent
                        ? 'Present'
                        : formatDate(certificate.endDate) || 'End'}
                    </span>
                  )}
                </div>
